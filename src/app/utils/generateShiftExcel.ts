...
function addHalfSheet(
  ...
  for (let pg = 0; pg < numPages; pg++) {
    ...
    // ── Row 1: Title ───────────────────────────────────────────────
    const titleText = `${p.departmentName}　${p.year}年${p.month}月${halfLabel}　シフト管理表　[Ver. 5.0]`;
...
function addFullSheet(workbook: ExcelJS.Workbook, p: ShiftExcelParams) {
  ...
  for (let pg = 0; pg < numPages; pg++) {
    ...
    // ── Row 1: Title ────────────────────────────────────────────────
    const titleText = `${p.departmentName}　${p.year}年${p.month}月　シフト管理表　[Ver. 5.0]`;
...
export async function generateShiftExcel(p: ShiftExcelParams): Promise<void> {
  ...
  workbook.creator = 'シフト管理システム Ver. 5.0';
